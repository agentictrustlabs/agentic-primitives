...
for (const chain of manifest.supportedChains) {
  // Addresses are unique per chain, not across chains — the same CREATE2 salt lands the same
  // contract at the same address on two chains, and that is a feature, not a duplicate.
