...
every new project — start there rather than forking Commons.

## What to hand it, in order

0. A project from `npx create-primitives-app` if you are starting a product — not a fork
   of Commons.
...
4. `apps/web/src/worker/index.ts` in a scaffold, or `apps/commons/src/worker/index.ts` in this
   repo — a complete, working example of every pattern.
